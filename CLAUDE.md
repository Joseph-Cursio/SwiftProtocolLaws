...
**ProtocolLawKit M1–M5 shipped (M4 second-backend partial). ProtoLawMacro M1 shipped.** The kit covers Equatable, Hashable, Comparable, Codable, IteratorProtocol, Sequence, Collection, SetAlgebra (PRD §4.3); the suppression / intentional-violation API (§4.7); the public `PropertyBackend` abstraction (§4.5) with `SwiftPropertyBasedBackend` as the default; the M5 confidence-reporting upgrade (replay environment validation, near-miss tracking, opt-in `CoverageClassifier`, formatter rendering); and the `@ProtoLawSuite` peer macro (PRD §5.3 Macro Mode) — applied to a type, reads its inheritance clause, emits a peer test struct of `checkXxxProtocolLaws` calls.

**Macro shape pivot from PRD §5.3.** The PRD's `@ProtoLawSuite(types: [Foo.self, Bar.self])`-on-a-test-suite form can't be implemented as a Swift macro: macro implementations see only their decoratee's syntax, not the surrounding file (this is the same file-locality limitation PRD §9 Decision 4 names for whole-module discovery). M1 ships a peer-macro-on-the-type variant that's actually achievable; the PRD's multi-type-on-a-suite form lives on the M2 Swift Package Plugin side.
